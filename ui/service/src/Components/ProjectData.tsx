...
const pages = ['dashboard', 'reports', 'test_suites']
        ...
        {(project) => (
          <>
            ...
            >
              <Grid item xs={12}>
                <TextWithCopyIcon
                  showText={`project id: ${project.id}`}
                  copyText={project.id}
            ...
            <Tabs value={pageIndex} aria-label="simple tabs example" indicatorColor={'primary'}>
              <Link component={RouterLink} to={`/projects/${projectId}`}>
                <Tab label="Dashboard" {...a11yProps(0)} value={'dashboard'} />
              </Link>
              <Link component={RouterLink} to={`/projects/${projectId}/reports`}>
                <Tab label="Reports" {...a11yProps(1)} value={'reports'} />
              </Link>
              <Link component={RouterLink} to={`/projects/${projectId}/test_suites`}>
                <Tab label="Test Suites" {...a11yProps(2)} value={'test_suites'} />
              </Link>
              <Tab label="Comparisons" {...a11yProps(3)} disabled={true} />
            </Tabs>
            <TabPanel
              value={pageIndex}
              index={pages.findIndex((p) => p === 'dashboard' || p === '')}
            >
              <Grid container justifyContent={'flex-end'}>
                <Grid item>
                  <TextField
                    ...
                    defaultValue={
                      project.date_from === undefined
                        ? null
                        : formatDate(new Date(Date.parse(project.date_from)))
                    }
                    onChange={(ev) => {
                  ...
                  <TextField
                    ...
                    type="datetime-local"
                    defaultValue={
                      project.date_to === undefined
                        ? null
                        : formatDate(new Date(Date.parse(project.date_to)))
                    }
                    onChange={(ev) => {
                      let value = ev.target.value
                      console.log(ev)
            ...
            <TabPanel value={pageIndex} index={pages.findIndex((p) => p === 'reports')}>
              ...
              {reportId ? (
                <ReportViewer projectId={projectId!} reportId={reportId} />
              ) : (
