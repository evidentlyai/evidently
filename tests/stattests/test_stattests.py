...
from evidently.calculations.stattests.mann_whitney_urank_stattest import (
...
@pytest.mark.parametrize(
    ...
    (
        (
            pd.Series([1, 1, 1, 1, 1] * 5, dtype="float"),
            pd.Series([0, 0, 0, 0, 0] * 5, dtype="float"),
            0.1,
            0,
            True,
        ),
        ...
        (
            pd.Series(np.random.normal(0, 0.5, 100)),
            pd.Series(np.random.normal(0, 0.1, 100)),
            0.1,
            0,
            True,
        ),
...
def test_tvd_stattest(reference, current, threshold, pvalue, drift_detected) -> None:
    assert tvd_test.func(reference, current, "cat", threshold) == (
        pvalue,
        drift_detected,
    )
...
def test_epps_singleton() -> None:
    reference = pd.Series([1, 2, 3, 4, 5, 6]).repeat([16, 18, 16, 14, 12, 12])
    current = pd.Series([1, 2, 3, 4, 5, 6]).repeat([16, 16, 16, 16, 16, 8])
    assert epps_singleton_test.func(reference, current, "num", 0.05) == (
        approx(0.81, abs=1e-2),
        False,
    )
...
def test_t_test() -> None:
    reference = pd.Series([38.7, 41.5, 43.8, 44.5, 45.5, 46.0, 47.7, 58.0])
    current = pd.Series([39.2, 39.3, 39.7, 41.4, 41.8, 42.9, 43.3, 45.8])
    assert t_test.func(reference, current, "num", 0.05) == (
        approx(0.084, abs=1e-3),
        False,
    )
