...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            ColumnMapping(),
            None,
            BaseWidgetInfo(type="big_graph", title="Target Drift: not detected, p_value=1.0", size=2),
        ),
        (
            pd.DataFrame({"target": [1, 2, 3, 4]}),
            pd.DataFrame({"target": [1, 2, 3, 1]}),
        ...
        (
            ...
            ColumnMapping(),
            "prediction",
            BaseWidgetInfo(type="big_graph", title="Prediction Drift: not detected, p_value=1.0", size=2),
        ),
        (
            pd.DataFrame({"data": [1, 2, 3, 4]}),
            pd.DataFrame({"data": [1, 2, 3, 1]}),
