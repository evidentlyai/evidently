...
@pytest.fixture
def widget() -> ProbClassPRTableWidget:
    options_provider = OptionsProvider()

    widget = ProbClassPRTableWidget("test_widget")
    widget.options_provider = options_provider
    return widget


def test_prob_class_pr_table_widget_analyzer_list(widget: ProbClassPRTableWidget) -> None:
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            "current",
            BaseWidgetInfo(type="big_table", title="test_widget", size=1),
        ),
    ),
)
def test_prob_class_pr_table_widget_simple_case(
    widget: ProbClassPRTableWidget,
