...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            pd.DataFrame({"test": [1, 2, np.inf]}),
            {"test": {0: 1.0, 1: 2.0, 2: approx(np.nan, nan_ok=True)}},
        ),
        (
            pd.DataFrame({"test": [np.NAN, np.inf, -np.inf]}),
            {
                "test": {
                    0: approx(np.nan, nan_ok=True),
                    1: approx(np.nan, nan_ok=True),
                    2: approx(np.nan, nan_ok=True),
                }
            },
        ),
        (
