...
def test_data_integrity_test_share_of_missing_values() -> None:
    ...
    suite = TestSuite(
        tests=[TestShareOfMissingValues(missing_values=["", "null"], lt=0.4)]
    )
    suite.run(
        current_data=test_dataset, reference_data=None, column_mapping=ColumnMapping()
    )
    ...
    suite = TestSuite(
        tests=[
            TestShareOfMissingValues(
                missing_values=["", "null", False, None], replace=True, lt=0.4
            )
        ]
    )
    suite.run(
...
def test_data_integrity_test_different_missing_values_one_column_with_defaults() -> None:
    ...
    suite = TestSuite(
        tests=[
            TestColumnNumberOfDifferentMissingValues(
                column_name="feature", missing_values=["null", "n/a"], replace=False
            )
        ]
    )
    suite.run(
