...
def check_profile_section_result_common_part(
    section_result: dict, section_result_name: str
) -> None:
...
def check_section_without_calculation_results(
    profile_section_class: Type[ProfileSection], part_id: str
) -> None:
...
def calculate_section_results(
    ...
    for analyzer_class in profile_section.analyzers():
        ...
        analyzers_results[analyzer_class] = analyzer.calculate(
            reference_data, current_data, columns_mapping
        )

    profile_section.calculate(
        reference_data, current_data, columns_mapping, analyzers_results
    )
