...
def test_data_integrity_metrics_different_null_values() -> None:
    test_dataset = pd.DataFrame(
        {
            "category_feature": ["", "n/a", "3"],
            "numerical_feature": [3, -9999, 0],
            "target": [None, np.NAN, 1],
    ...
    assert result.current_null_values.number_of_differently_encoded_nulls == 1
    assert result.current_null_values.number_of_null_values == 1
    assert result.reference_null_values is None
    ...
    assert result.current_null_values.number_of_null_values == 7
    assert result.reference_null_values is None
