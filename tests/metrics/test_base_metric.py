...
def test_metric_generator():
    ...
    report = Report(
        metrics=[generate_column_metrics(DataQualityValueRangeMetrics, parameters={"left": 0, "right": 10})]
    )
    ...
    report = Report(
        metrics=[
            generate_column_metrics(
                metric_class=DataQualityValueRangeMetrics, columns=["col2", "col3"], parameters={"left": 0, "right": 10}
            )
        ]
    )
    report.run(current_data=test_data, reference_data=None)
