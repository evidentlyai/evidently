...
| `TextOverviewPreset(column_name=”text”)` | Evaluates data drift and descriptive statistics for text data. <br><br>Input features (text) are required.<br><br>**Contents**:<br>`ColumnSummaryMetric()`<br>`TextDescriptorsDistribution()`<br>`TextDescriptorsCorrelation()`<br> With reference, also: `ColumnDriftMetric`<br>`TextDescriptorsDriftMetric()`| **Required**:<br>`column_name` |
