...
| **DoesNotContain()** | Checks if the text does not contain any or all specified items. (Scale: Binary) | **Required:**<br>`items`: List[str] <br><br>**Optional:**<ul><li>`display_name`</li><li>`mode = 'all'` (available: `'any'`)</li><li>`case_sensitive = True` (available: `False`)</li></ul> |
| **IncludesWords()** | Checks if the text includes any (default) or all specified words. By default, considers inflected and variant forms of the same word. (Scale: Binary) | **Required:**<br>`words_list`: List[str] <br><br>**Optional:**<ul><li>`display_name`</li><li>`mode = 'any'` (available: `'all'`)</li><li>`lemmatize = True` (available: `False`)</li></ul> |
| **ExcludesWords()** | Checks if the text excludes all specified words. By default, considers inflected and variant forms of the same word. (Scale: Binary) | **Required:**<br>`words_list`: List[str] <br><br>**Optional:**<ul><li>`display_name`</li><li>`mode = 'all'` (available: `'any'`)</li><li>`lemmatize = True` (available: `False`)</li></ul> |
