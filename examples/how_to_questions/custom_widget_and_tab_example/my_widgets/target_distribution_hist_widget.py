...
class TargetDistributionHistWidget(Widget):
    ...
    def calculate(
        ...
        fig = ff.create_distplot(
            hist_data, group_labels, colors=colors, show_rug=False, show_curve=False, bin_size=0.25
        )
        ...
        target_dist_hist_json = json.loads(fig.to_json())

        return BaseWidgetInfo(
            title=self.title,
            type="big_graph",
            size=1,
            params={"data": target_dist_hist_json["data"], "layout": target_dist_hist_json["layout"]},
        )
