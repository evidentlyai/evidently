...
logging.basicConfig(level=logging.INFO, format="SIMULATE - %(levelname)s - %(message)s")
...
def simulate() -> None:
    ...
    # Load data
    additional_columns: List[Text] = ["lpep_pickup_datetime", "uuid", "duration_min"]
    num_features: List[Text] = DATA_COLUMNS["num_features"]
    cat_features: List[Text] = DATA_COLUMNS["cat_features"]
    required_columns: List[Text] = additional_columns + num_features + cat_features
    ...
    while total_rows_taken <= test_data.shape[0]:
        ...
        if resp.status_code == 200:
            ...
            pred_columns: List[Text] = ["lpep_pickup_datetime", "uuid", "predictions"]
