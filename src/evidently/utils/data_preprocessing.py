...
def _process_column(
    ...
    column_type = (
        ...
        else _get_column_type(column_name, data, mapping)
    )
...
def create_data_definition(
        ...
        for (embedding_name, columns) in mapping.embeddings.items():
            ...
            for column in columns:
                ...
                if presence != ColumnPresenceState.Present:
                    logging.warning(
                        f"Column {column} isn't present in data. Skipping it."
                    )
                else:
    ...
    col_defs = [
        _process_column(column_name, data, if_partially_present="skip", mapping=mapping)
    ...
    if mapping.numerical_features is None:
        num = [
            ...
            and _column_not_present_in_list(
                column.column_name, embedding_columns, "skip", ""
            )
        ]
    ...
    if mapping.text_features is not None:
        all_columns.extend(
            [
                _process_column(
                    column_name, data, predefined_type=ColumnType.Text, mapping=mapping
                )
    ...
    if ref_type is not None and cur_type is not None:
        if ref_type != cur_type:
            ...
            if ref_type.kind not in available_set or cur_type.kind not in available_set:
                ...
                cur_type = ref_type
            if not np.can_cast(cur_type, ref_type) and not np.can_cast(
                ref_type, cur_type
            ):
                logging.warning(
                    f"Column {column_name} have different types in reference {ref_type} and current {cur_type}."
    ...
    if mapping is not None and (
        ...
        reg_condition = mapping.task == "regression" or (
            pd.api.types.is_numeric_dtype(
                cur_type if cur_type is not None else ref_type
            )
