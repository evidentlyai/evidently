...
from evidently.base_metric import ColumnName, ColumnNotFound, DatasetType, InputData, Metric
...
from evidently.spark.base import SparkDataFrame, SparkSeries
...
class SparkInputData(InputData):
    ...
    @staticmethod
    def _get_by_column_name(dataset: SparkDataFrame, additional: SparkDataFrame, column: ColumnName, add_columns: Optional[List[str]] = None) -> SparkSeries:
    ...
    def get_reference_column(self, column: Union[str, ColumnName], datetime_column: Optional[str] = None) -> Optional[SparkSeries]:
...
class SparkEngine(Engine["SparkMetricImplementation", SparkInputData]):
    def convert_input_data(self, data: InputData) -> SparkInputData:
        if not isinstance(data.current_data, SparkDataFrame) or (data.reference_data is not None and not isinstance(data.reference_data, SparkDataFrame)):
