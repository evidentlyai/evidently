...
from evidently.calculations.stattests.registry import PossibleStatTestType, StatTest, StatTestData, StatTestFuncReturns, StatTestImpl, \
    TStatTestData, \
    get_registered_stattest, add_stattest_impl
...
class SparkStatTestImpl(StatTestImpl[SpartStatTestData, SparkEngine]):
    ...
    base_stat_test: StatTest = None

    def __call__(self, data: SpartStatTestData, feature_type: ColumnType, threshold: Optional[float]) -> StatTestFuncReturns:
        raise NotImplementedError

    def __init_subclass__(cls):
        if cls.base_stat_test is not None:
...
def _get_default_stattest(reference_data: SparkSeries,
                          current_data: SparkSeries,
                          feature_type: ColumnType, ) -> StatTest:
