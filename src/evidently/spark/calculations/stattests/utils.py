...
def get_binned_data(
    reference_data: SparkSeries, current_data: SparkSeries, column_name: str, feature_type: ColumnType, fill_zeroes: bool = True
):
    ...
    if feature_type == ColumnType.Numerical:# and n_vals > 20:
        bins, dmax, dmin = hist_bin_doane(current_data.dropna().union(reference_data.dropna()), column_name)
        reference_percents = get_histogram(reference_data, column_name=column_name, nbinsx=bins, density=False, dmax=dmax, dmin=dmin)[0] / reference_data.count()
        current_percents = get_histogram(current_data, column_name=column_name, nbinsx=bins, density=False, dmax=dmax, dmin=dmin)[0]  / current_data.count()

    else:
        ref_stats = reference_data.groupby(column_name).agg(sf.count().alias("ref"))
        cur_stats = current_data.groupby(column_name).agg(sf.count().alias("cur"))
