...
class OOVWordsPercentage(GeneratedFeature):
    ...
    def __init__(
        self, column_name: str, ignore_words=(), display_name: Optional[str] = None
    ):
        ...
        def oov_share(s, ignore_words=()):
            ...
            for word in words_:
                if (
                    ...
                    and self._lem.lemmatize(word.lower()) not in self._eng_words
                ):
                    oov_num += 1
    ...
    def feature_name(self):
        return additional_feature(
            self,
            self.column_name,
            self.display_name or f"OOV Words % for {self.column_name}",
        )
