...
from evidently.renderers.render_utils import get_distribution_plot_figure, plot_distr
...
class TestHighlyCorrelatedColumns(BaseDataQualityCorrelationsMetricsValueTest):
    ...
    def get_condition(self) -> TestValueCondition:
        ...
        if reference_correlation is not None:
            value = reference_correlation.stats[
...
class TestColumnValueMinRenderer(TestRenderer):
    def render_html(self, obj: TestColumnValueMin) -> TestHtmlInfo:
        ...
        fig = plot_distr(
            hist_curr=curr_distr, hist_ref=ref_distr, color_options=self.color_options
        )
        ...
        if min_value is not None:
            fig = plot_metric_value(
                fig, float(min_value), f"current {column_name} min value"
            )
        info.with_details(
            f"Min Value {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestColumnValueMaxRenderer(TestRenderer):
    def render_html(self, obj: TestColumnValueMax) -> TestHtmlInfo:
        ...
        if max_value is not None:
            fig = plot_metric_value(
                fig, float(max_value), f"current {column_name} max value"
            )
        info.with_details(
            f"Max Value {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestColumnValueMeanRenderer(TestRenderer):
    def render_html(self, obj: TestColumnValueMean) -> TestHtmlInfo:
        ...
        info.with_details(
            f"Mean Value {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestColumnValueMedianRenderer(TestRenderer):
    def render_html(self, obj: TestColumnValueMedian) -> TestHtmlInfo:
        ...
        if percentile_50 is not None:
            fig = plot_metric_value(
                fig, percentile_50, f"current {column_name} median value"
            )
        info.with_details(
            f"Median Value {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestMeanInNSigmasRenderer(TestRenderer):
    def render_json(self, obj: TestMeanInNSigmas) -> dict:
        ...
        if metric_result.reference_characteristics is not None:
            ...
            base["parameters"][
                "reference_mean"
            ] = metric_result.reference_characteristics.mean
            base["parameters"][
                "reference_std"
            ] = metric_result.reference_characteristics.std
        return base
...
class TestValueRange(Test):
    ...
    def __init__(
        ...
        self.left = left
        self.right = right
        self.metric = ColumnValueRangeMetric(
            column_name=column_name, left=left, right=right
        )
...
class TestValueRangeRenderer(TestRenderer):
    def render_html(self, obj: TestValueRange) -> TestHtmlInfo:
        ...
        info.with_details(
            f"Value Range {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestShareOfOutRangeValuesRenderer(TestRenderer):
    ...
    def render_html(self, obj: TestShareOfOutRangeValues) -> TestHtmlInfo:
        ...
        fig = plot_check(fig, obj.condition, color_options=self.color_options)
        info.with_details(
            f"Share Out of Range for {column_name}", plotly_figure(title="", figure=fig)
        )
        return info
...
class TestValueListRenderer(TestRenderer):
    def render_json(self, obj: TestValueList) -> dict:
        ...
        base["parameters"][
            "number_not_in_list"
        ] = obj.metric.get_result().current.number_not_in_list
        return base
    ...
    def render_html(self, obj: TestValueList) -> TestHtmlInfo:
        ...
        if metric_result.reference is not None:
            ref_df = pd.DataFrame(
                metric_result.reference.values_in_list.items(), columns=["x", "count"]
            )
        ...
        additional_plots = plot_value_counts_tables(
            column_name, values, curr_df, ref_df, "value_list"
        )
        info.details = additional_plots
        return info
...
class TestNumberOfOutListValuesRenderer(TestRenderer):
    def render_html(self, obj: TestNumberOfOutListValues) -> TestHtmlInfo:
        ...
        additional_plots = plot_value_counts_tables(
            column_name, values, curr_df, ref_df, "number_value_list"
        )
        info.details = additional_plots
        return info
...
class TestShareOfOutListValuesRenderer(TestRenderer):
    ...
    def render_html(self, obj: TestShareOfOutListValues) -> TestHtmlInfo:
        ...
        additional_plots = plot_value_counts_tables(
            column_name, values, curr_df, ref_df, "share_value_list"
        )
        info.details = additional_plots
        return info
