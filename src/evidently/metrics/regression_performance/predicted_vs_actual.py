...
class RegressionPredictedVsActualScatterResults(MetricResult):
    ...
    current_raw, current_agg = raw_agg_properties(
        "current", PredActualScatter, AggPredActualScatter, False
    )
    reference_raw, reference_agg = raw_agg_properties(
        "reference", PredActualScatter, AggPredActualScatter, True
    )
...
class RegressionPredictedVsActualScatter(
    ...
    def calculate(self, data: InputData) -> RegressionPredictedVsActualScatterResults:
        ...
        curr_df = self._make_df_for_plot(
            curr_df.copy(), target_name, prediction_name, None
        )
        ...
        if (
            ...
            or (
                ref_df is not None
                and not is_possible_contour(
                    ref_df[prediction_name], ref_df[target_name]
                )
            )
        ):
            curr_df.drop_duplicates(subset=[prediction_name, target_name], inplace=True)
            current_scatter = PredActualScatter(
                predicted=curr_df[prediction_name], actual=curr_df[target_name]
            )
            ...
            if ref_df is not None:
                ...
                reference_scatter = PredActualScatter(
                    predicted=ref_df[prediction_name], actual=ref_df[target_name]
                )
        ...
        current_agg = AggPredActualScatter(
            data=get_gaussian_kde(curr_df[prediction_name], curr_df[target_name])
        )
        reference_agg = AggPredActualScatter(data=None)
        if ref_df is not None:
            reference_agg = AggPredActualScatter(
                data=get_gaussian_kde(ref_df[prediction_name], ref_df[target_name])
            )
        return RegressionPredictedVsActualScatterResults(
            current=current_agg, reference=reference_agg, agg_data=True
        )
...
class RegressionPredictedVsActualScatterRenderer(MetricRenderer):
    def render_raw(
        self, current: PredActualScatter, reference: Optional[PredActualScatter]
    ):
        fig = plot_scatter(
    ...
    def render_agg(
        self, current: AggPredActualScatter, reference: Optional[AggPredActualScatter]
    ):
