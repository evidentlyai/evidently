...
def apply_func_to_binned_data(
    ...
    if is_ref_data:
        reference = IntervalSeries.from_data(
            df_for_bins[df_for_bins.data_storage == "ref"].groupby("target_binned", observed=False).apply(_apply)
        )

    result = RegressionMetricScatter(
        current=IntervalSeries.from_data(
            df_for_bins[df_for_bins.data_storage == "curr"].groupby("target_binned", observed=False).apply(_apply)
        ),
        reference=reference,
    )
