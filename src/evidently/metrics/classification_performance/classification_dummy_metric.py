...
class ClassificationDummyMetric(ThresholdClassificationMetric[ClassificationDummyMetricResults]):
    ...
    def calculate(self, data: InputData) -> ClassificationDummyMetricResults:
        ...
        if (
            ...
            if self.threshold is not None or self.k is not None:
                ...
                threshold = 0.5
            current_dummy = self.correction_for_threshold(current_dummy, threshold, target, labels,
        ...
        if data.reference_data is not None:
            ...
            if (
                ...
                by_reference_dummy = self.correction_for_threshold(by_reference_dummy, threshold, target, labels,
                                                                   prediction.prediction_probas.shape)
            if (
                prediction is not None
                and prediction.prediction_probas is not None
    ...
    def correction_for_threshold(self, dummy_results: DatasetClassificationQuality, threshold: float, target: pd.Series,
                                 labels: list, probas_shape: tuple):
        if threshold == 1.0:
            coeff_precision = 1.0
        else:
