...
def get_binned_data(
    ...
    if feature_type == ColumnType.Numerical and n_vals > 20:
        bins = np.histogram_bin_edges(
            pd.concat([reference_data, current_data], axis=0).values, bins="sturges"
        )
...
def permutation_test(
    reference_data, current_data, observed, test_statistic_func, iterations=100
):
    ...
    for i in range(iterations):
        ...
        new_reference = np.random.choice(
            combined_data, len(reference_data), replace=False
        ).tolist()
        ...
        hold_test_statistic.append(
            test_statistic_func(pd.Series(new_reference), pd.Series(new_current))
        )

    p_val = sum(observed <= abs(np.array(hold_test_statistic))) / len(
        hold_test_statistic
    )
    return p_val
...
def generate_fisher2x2_contingency_table(
    ...
    unique_categories = set(
        reference_data.unique().tolist() + current_data.unique().tolist()
    )
