...
def create_data():
    ...
    for name, rs in (("TheOtherStore", 0), ("AMajorCompetitor", 42), ("AwesomeShop", 100)):
        ...
        reviews.loc[random_index, 'Review_Text'] = reviews.loc[random_index, 'Review_Text'] + f' mention competitor {name}'

    np.random.seed(13)
    random_index = np.random.choice(reviews.index, 1000, replace=False)
    reviews.loc[random_index, 'Review_Text'] = reviews.loc[random_index, 'Review_Text'] + ' mention www.someurl.someurl '
...
def create_report(i: int, data):
    ...
    text_report = Report(
        metrics=[
            ...
            metrics.ColumnSummaryMetric(column_name=descriptors.NonLetterCharacterPercentage(
            ...
            metrics.ColumnSummaryMetric(column_name=descriptors.RegExp(
                display_name="urls", reg_exp='.*(http|www)\S+.*').for_column("Review_Text")
            ),
